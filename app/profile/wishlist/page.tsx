...
export default function WishlistPage() {
  ...
  const loadWishlist = async () => {
    try {
      ...
      const wishIds: string[] = wishIdsRaw ? JSON.parse(wishIdsRaw) : []
      const filtered = allProducts.filter((p) => wishIds.includes(p.id))
  ...
  const handleRemove = (id: string) => {
    const wishIdsRaw = localStorage.getItem('cc_wishlist')
    const wishIds: string[] = wishIdsRaw ? JSON.parse(wishIdsRaw) : []
    const updatedIds = wishIds.filter((item) => item !== id)
    localStorage.setItem('cc_wishlist', JSON.stringify(updatedIds))
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-md p-6 min-h-[400px]">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                <Heart className="w-6 h-6 text-red-500 fill-red-500" /> Saved Products
              </h2>
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {wishlistProducts.map((product) => {
                    const discountedPrice = product.discount
                      ...
                      : product.price
                    return (
                      <div key={product.id} className="border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition flex flex-col justify-between bg-white">
                        <div>
                          {/* Image */}
                          <div className="h-40 bg-gray-100 overflow-hidden relative">
                            ...
                            />
                            {product.discount && (
                              <div className="absolute top-2 right-2 bg-red-500 text-white font-bold text-xs px-2 py-0.5 rounded-full">
                                SALE
                              </div>
                            )}
                          </div>
                          ...
                          <div className="p-4">
                            ...
                            <h3 className="font-semibold text-gray-900 line-clamp-1 mt-0.5">
                              {product.name}
                            </h3>
                            <p className="text-xs text-green-600 capitalize font-medium">{product.category}</p>
                        ...
                        <div className="border-t p-3 bg-gray-50 flex gap-2">
                          <Link
                            ...
                            className="flex-1 bg-white hover:bg-gray-100 text-gray-700 font-bold py-2 border rounded-lg transition flex items-center justify-center gap-1.5 text-xs"
                          >
                          ...
                          <button
                            ...
                            className="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold py-2 rounded-lg transition flex items-center justify-center gap-1.5 text-xs disabled:opacity-50"
                          >
                ...
                <div className="text-center py-20">
                  ...
                  <p className="text-gray-400 text-sm mt-1 mb-6">Mark products you love to save them for later.</p>
