  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const localUpdate = { ...user, ...formData }
        setUser(localUpdate)
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" name="first_name" value={formData.first_name} onChange={handleChange}
                  ...
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none" required />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea name="address" rows={3} value={formData.address} onChange={handleChange}
                ...
                className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none text-sm" required></textarea>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input type="text" name="pincode" value={formData.pincode} onChange={handleChange}
                  placeholder="6-digit postal code" maxLength={6}
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none" required />
              </div>
            </div>
            <div className="pt-6 flex gap-4">
              <button type="submit" disabled={loading}
                ...
                {loading ? (<><Loader2 className="w-5 h-5 animate-spin" />Saving Changes...</>) : ('Save Profile')}
              </button>
              <Link href="/profile"
